...
class ForumController extends AbstractController
{
    ...
    public function newPost(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            foreach ($imageFiles as $imageFile) {
                ...
                $image = $this->createImageFromStoredFile($newFilename, $postsDir, $position);
                $post->addImage($image);
                $position++;
            }
            ...
            $em->persist($post);
            $em->flush();
            $this->mirrorPostImagesToJavaBlobColumn($post, $connection);
    ...
    public function editPost(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->applyMediaOrdering(
                ...
                function (string $filename, int $pos) use ($post, $postsDir) {
                    $img = $this->createImageFromStoredFile($filename, $postsDir, $pos);
                    $post->addImage($img);
                }
            );
    ...
    private function createImageFromStoredFile(string $filename, string $targetDir, int $position): Image
    {
        $image = (new Image())
        ...
        $image
            ->setImageBlob($data)
            ->setMimeType($this->guessImageMimeType($path));

        return $image;
    }

    private function mirrorPostImagesToJavaBlobColumn(Post $post, Connection $connection): void
    {
        $this->ensureJavaMediaDataColumn($connection);
        ...
        foreach ($post->getImages() as $image) {
            ...
            try {
                $connection->executeStatement(
                    'UPDATE sf_images SET media_data = :media_data, mime_type = COALESCE(mime_type, :mime_type) WHERE id = :id',
                    [
                        'media_data' => $data,
                    ...
                    [
                        'media_data' => ParameterType::LARGE_OBJECT,
                        'mime_type' => ParameterType::STRING,
                        'id' => ParameterType::INTEGER,
                    ]
                );
            } catch (\Throwable) {
                // The web post remains valid; this mirror only keeps Java in sync.
            }
        }
    }

    private function ensureJavaMediaDataColumn(Connection $connection): void
    {
        try {
            $exists = (int) $connection->fetchOne(
                'SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = :table AND COLUMN_NAME = :column',
                ['table' => 'sf_images', 'column' => 'media_data']
            );

            if ($exists === 0) {
                $connection->executeStatement('ALTER TABLE sf_images ADD COLUMN media_data LONGBLOB NULL');
            }
        } catch (\Throwable) {
            // Ignore schema-sync errors here; the normal Symfony upload still works.
        }
    }
